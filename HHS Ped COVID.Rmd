...
state_ma_ch %>% 
  filter( hospital_name %in% ne_chs & collection_date > '2021-12-13') %>%
...
```{r, fig.height=4}
state_ma %>%
  filter( state %in% R1states & collection_date > '2021-11-30') %>%
...
```{r, fig.height=4}
state_ma %>%
  filter( state %in% c("NY") & collection_date > '2021-10-31') %>%
  select(state  
            , collection_date
            , admission_child_covid
            )
...
```{r, fig.height=4}
region_ma %>%
  filter(fema_region %in% c("Region 1", "Region 4", "Region 6", "Region 9") & collection_date > '2021-12-01') %>%
...
```{r}
us_ma %>%
  filter(collection_date > '2021-12-15') %>%
