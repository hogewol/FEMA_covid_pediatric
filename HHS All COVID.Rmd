...
```{r, fig.height=4}
state_d %>%
  filter( state %in% R1states & collection_date > '2022-01-10') %>% 
...
```{r, fig.height=4}
# IMPORTANT
date_most_hospitals_reported <- '2022-01-12'

state_d %>%
  # filter( state %in% R1states & total_hosp_reporting > 5 & collection_date > '2020-09-01') %>% 
  filter( state %in% R1states & collection_date > '2020-09-01' & collection_date <= date_most_hospitals_reported) %>% 
  mutate(r = staffing_shortage_yes/total_hosp_reporting) %>%
...
```{r, fig.height=4}
daily_cases_hosp %>%
  filter(state %in% R1states & collection_date > '2020-09-15') %>% 
  mutate(r = (daily_cases/Population)*100000) %>%
  ggplot(aes(x = collection_date, y = r)) + #, color = state
    facet_wrap(~ state, ncol = 2) + #scales = 'free_y' - For different axes
    # geom_line(color = "blue", size = 1) +
    geom_point(color = "gray45", size = 1) +
    geom_smooth(color = "blue", size = 1, span = 0.02) +
...
```{r, fig.height=4}
daily_cases_hosp_rates_full %>%
  filter(state %in% R1states & date > '2021-05-01') %>%
  mutate(r = weekly_confirmed_covid_admissions/lag_cases) %>%
  filter(r < 2) %>%
  ggplot(aes(x = date, y = r)) + #, color = state
    facet_wrap(~ state, ncol = 2) + #scales = 'free_y' - For different axes
...
```{r}
daily_cases_hosp_rates_full %>%
  filter(state %in% R1states & date > '2022-01-01' ) %>% #& date < '2021-10-01'
...
state_ma %>%
  filter(state %in% R1states & collection_date > '2022-01-11') %>%
  select(
    state
    , collection_date
    , total_utilization
