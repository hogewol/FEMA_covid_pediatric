...
```{r}
df_population <- read_excel('data/NST-EST2021-POP.xlsx', range = 'A10:D60', col_names = c('State Name', 'x1', 'x2', 'Population'))
```

Clean up/Select Data
```{r}
df_population$`State Name` <- gsub(".", "", as.character(df_population$`State Name`), fixed = TRUE)
df_population$state <- state.abb[match(df_population$`State Name`, state.name)]
...
```{r}
temp <- facility %>%
  filter(state %in% R1states & collection_date == '2021-12-27')
...
```{r}
fname1 <- paste0('2021-12-27_staff_shortage.xlsx')
...
for (state_abbrv in R1states){
  tempState <- facility %>%
  filter(state %in% state_abbrv & collection_date == '2021-12-27')
...
```{r, fig.height=4}
state_d %>%
  filter( state %in% R1states & collection_date > '2020-09-01' & collection_date < '2022-01-04') %>%
  ...
  ggplot(aes(x = collection_date, y = r)) + #, color = state
    ...
    scale_x_date(breaks = seq.Date(as.Date('2020-11-01'), as.Date('2022-01-01'), '2 months'), date_labels = "%b %d %Y") +
    scale_y_continuous(labels=scales::percent #, limits = c(0, 1)
                       )
```

```{r}
max(state_d$collection_date)
# str(state_d)
```
...
* POWER POINT Graph *
Case Incidence per 100,000 Ppl
```{r, fig.height=4}
state_ma %>%
  filter( state %in% R1states & collection_date > '2021-12-14') %>%
...
Case Numbers (all, not just hospitalizations)
```{r, fig.height=4}
daily_cases_hosp %>%
  filter(state %in% R1states & collection_date > '2020-09-01' & collection_date < '2022-01-05') %>%
...
daily_cases_hosp %>% 
  ...
  drop_na(daily_cases) %>%
  group_by(state) %>%
  mutate(max_collection_date = max(collection_date), r = (daily_cases/Population)*100000) %>%
  filter(collection_date == max_collection_date) %>%
  # filter(collection_date == '2021-12-27') %>%
  select(state, collection_date, r)
```
...
```{r, fig.height=4}
region_ma %>%
  filter(fema_region %in% c("Region 1") & collection_date > '2020-12-14' & collection_date < '2020-12-30') %>% #& collection_date < '2020-12-25'
  mutate(r = (admission_all_covid/Population)*100000)
```
...
```{r}
us_ma %>%
  filter(collection_date > '2021-12-14') %>%
  mutate(r = (admission_all_covid / 329500000)*100000)
```

```{r}
us_ma %>%
  filter(collection_date > '2020-12-17' & collection_date < '2020-12-23') %>%
  mutate(r = (covid_deaths / 329500000)*100000)
```
